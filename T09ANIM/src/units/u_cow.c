...
static VOID KH6_UnitInit( kh6UNIT_COW *Uni, kh6ANIM *Ani )
{
  KH6_RndPrimLoad(&Uni->Cow, "bin/models/cow.obj");
}

static VOID KH6_UnitClose( kh6UNIT_COW *Uni, kh6ANIM *Ani )
{
  KH6_RndPrimFree(&Uni->Cow); 
}

static VOID KH6_UnitRender( kh6UNIT_COW *Uni, kh6ANIM *Ani )
{
  INT s = 0, i, j;
...
kh6UNIT * KH6_UnitCreateCow( VOID )
{
  ...
  if ((Uni = KH6_AnimUnitCreate(sizeof(kh6UNIT_COW))) == NULL)
    return NULL;
